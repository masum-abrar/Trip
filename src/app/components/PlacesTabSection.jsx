        ...
        place.id === placeId
          ? {
              ...
              reviews: place.reviews.map(review =>
                review.id === reviewId
                  ? { ...review, replies: [...review.replies, { id: Date.now(), text: replyText[reviewId], user: "You" }] }
                  : review
              ),
            }
          : place
        ...
        {touristPlaces.map(place => (
          <div key={place.id} className="border rounded-lg shadow-lg p-4 bg-white relative">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-bold text-black">{place.name}</h2>
            </div>
            ...
            <div className="mt-4 border-t pt-4">
              <h3 className="font-semibold text-black text-lg mb-2">Leave a Review:</h3>
              <textarea
              ...
              <div className="mt-3">
                <h3 className="font-semibold text-black">Rate (out of 10):</h3>
            ...
            {place.reviews.length > 0 && (
              <div className="mt-6">
                <h3 className="font-semibold text-lg mb-3 text-black">Recent Reviews:</h3>
                    ...
                    {review.replies.length > 0 && (
                      <div className="mt-2 pl-4 border-l">
                        {review.replies.map(reply => (
                          <p key={reply.id} className="text-sm text-gray-600">{reply.user}: {reply.text}</p>
                        ))}
                      </div>
                    )}
                  </div>
